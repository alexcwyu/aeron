...
#define AERON_COMMAND_RB_TYPE_PTR (1)
#define AERON_COMMAND_RB_TYPE_INLINE (2)

static int aeron_client_conductor_command_rb_offer(aeron_mpsc_rb_t *command_queue, void *cmd)
{
    int fail_count = 0;
    ...
    do
    {
        ...
        {
            AERON_SET_ERR(EINVAL, "%s", "could not offer to conductor command queue");
            return -1;
        }
...
static int aeron_client_conductor_command_rb_offer_inline(aeron_mpsc_rb_t *command_queue, void *cmd, size_t length)
{
    int fail_count = 0;

    do
    {
        aeron_rb_write_result_t result = aeron_mpsc_rb_write(command_queue, AERON_COMMAND_RB_TYPE_INLINE, cmd, length);
...
static int32_t aeron_client_conductor_add_registering_resource(
    aeron_client_conductor_t *conductor, aeron_client_registering_resource_t *resource)
{
    int result = 0;
...
static void aeron_client_conductor_rb_on_command(int32_t type, const void *ptr, size_t length, void *clientd)
{
    ...
    {
        aeron_client_conductor_on_command(clientd, (void *)ptr);
    }
}

static void aeron_client_conductor_async_resource_free(aeron_client_registering_resource_t *async_resource)
{
    if (NULL != async_resource)
    {
        aeron_free(async_resource->error_message);
        aeron_free(async_resource->uri);

        if (AERON_CLIENT_MANAGED_RESOURCE_TYPE_COUNTER == async_resource->type ||
            AERON_CLIENT_MANAGED_RESOURCE_TYPE_STATIC_COUNTER == async_resource->type)
        {
            aeron_free((void *)async_resource->counter.key_buffer);
            aeron_free((void *)async_resource->counter.label_buffer);
        }

        aeron_free(async_resource);
    }
}

static void aeron_client_conductor_drain_pending_cmd_on_close(
    int32_t type, const void *ptr, size_t length, void *clientd)
{
    const aeron_client_conductor_t *conductor = clientd;

    if (AERON_COMMAND_RB_TYPE_INLINE == type)
    {
        aeron_client_remove_resource_cmd_t *cmd = (aeron_client_remove_resource_cmd_t *)ptr;
    ...
    size_t last_index)
{
    ...
    aeron_client_conductor_on_resource_registration_error(
        resource, AERON_ERROR_CODE_GENERIC_ERROR, strlen(err), err);
    ...
    for (size_t i = 0, size = conductor->registering_resources.length; i <  size; i++)
    {
            ...
            response->correlation_id == resource->registration_id)
        {
            ...
            if (aeron_counter_create(
                ...
                counter_addr) < 0)
            {
                aeron_client_conductor_on_counter_ready_error(conductor, resource);
                return -1;
            }
...
static int aeron_client_conductor_check_liveness(aeron_client_conductor_t *conductor, int64_t now_ns)
{
    if (now_ns > (conductor->time_of_last_keepalive_ns + (int64_t)conductor->keepalive_interval_ns))
    {
        ...
        if (now_ms > (last_keepalive_ms + (int64_t)conductor->driver_timeout_ms))
        {
            conductor->is_terminating = true;
            aeron_client_conductor_force_close_resources(conductor);
...
static int aeron_client_conductor_on_cmd_add_publication(void *clientd, void *item)
{
    ...
    if (aeron_client_conductor_add_registering_resource(conductor, async) < 0)
    {
        AERON_APPEND_ERR("%s", "");
        return -1;
    }
    ...
    int32_t offset = aeron_client_conductor_try_claim_driver_command(conductor, AERON_COMMAND_ADD_PUBLICATION, command_length);
    if (offset < 0)
    {
...
static int aeron_client_conductor_on_cmd_add_exclusive_publication(void *clientd, void *item)
{
    ...
    int32_t offset = aeron_client_conductor_try_claim_driver_command(conductor, AERON_COMMAND_ADD_EXCLUSIVE_PUBLICATION, command_length);
    if (offset < 0)
    {
        const char* msg = "failed to send command to the driver";
        aeron_client_conductor_on_resource_registration_error(
...
static int aeron_client_conductor_on_cmd_add_subscription(void *clientd, void *item)
{
    ...
    if (aeron_client_conductor_add_registering_resource(
        conductor, async) < 0)
    {
        AERON_APPEND_ERR("%s", "");
        return -1;
    ...
    int32_t offset = aeron_client_conductor_try_claim_driver_command(conductor, AERON_COMMAND_ADD_SUBSCRIPTION, command_length);
    if (offset < 0)
    {
        const char* msg = "failed to send command to the driver";
        aeron_client_conductor_on_resource_registration_error(
...
static int aeron_client_conductor_on_cmd_add_counter(void *clientd, void *item)
{
    ...
    int32_t offset = aeron_client_conductor_try_claim_driver_command(conductor, AERON_COMMAND_ADD_COUNTER, command_length);
    if (offset < 0)
    {
        const char* msg = "failed to send command to the driver";
        aeron_client_conductor_on_resource_registration_error(
...
static int aeron_client_conductor_on_cmd_add_static_counter(void *clientd, void *item)
{
    ...
    int32_t offset = aeron_client_conductor_try_claim_driver_command(conductor, AERON_COMMAND_ADD_STATIC_COUNTER, command_length);
    if (offset < 0)
    {
        const char* msg = "failed to send command to the driver";
        aeron_client_conductor_on_resource_registration_error(
    ...
    int (*destination_func)(void *clientd, void *command))
{
    ...
    *async = NULL;
    ...
    cmd->command_base.item = conductor;
    ...
    {
        if (aeron_client_conductor_command_rb_offer(conductor->command_rb, cmd) < 0)
        {
            goto error;
        }
    }
...
static int aeron_client_conductor_on_cmd_get_next_available_session_id(void *clientd, void *item)
{
    ...
    if (conductor->control_protocol_version >= aeron_semantic_version_compose(1, 0, 0))
    {
        ...
        int32_t offset = aeron_client_conductor_try_claim_driver_command(conductor, AERON_COMMAND_GET_NEXT_AVAILABLE_SESSION_ID, command_length);
        if (offset < 0)
        {
            const char* msg = "failed to send command to the driver";
            aeron_client_conductor_on_resource_registration_error(
    ...
    {
        async->resource.next_session_id = aeron_randomised_int32();
        AERON_SET_RELEASE(async->registration_status, AERON_CLIENT_REGISTRATION_STATUS_REGISTERED);
        return 0;
    }
}
...
int aeron_client_conductor_init(aeron_client_conductor_t *conductor, aeron_context_t *context)
{
    ...
    conductor->command_rb = &context->command_rb;
...
int aeron_client_conductor_do_work(aeron_client_conductor_t *conductor)
{
    ...
    work_count += (int)aeron_mpsc_rb_read(
        conductor->command_rb, aeron_client_conductor_rb_on_command, conductor, 1);
...
void aeron_client_conductor_on_close(aeron_client_conductor_t *conductor)
{
    ...
    for (size_t i = 0, size = conductor->registering_resources.length; i <  size; i++)
    {
        aeron_client_conductor_async_resource_free(conductor->registering_resources.array[i].resource);
    }
    ...
    void *on_close_complete_clientd)
{
    ...
    if (aeron_client_conductor_command_rb_offer(conductor->command_rb, publication) < 0)
    {
    ...
    void *on_close_complete_clientd)
{
    ...
    if (aeron_client_conductor_command_rb_offer(conductor->command_rb, subscription) < 0)
    {
...
static int aeron_client_conductor_on_cmd_remove_resource(void *clientd, void *item)
{
    ...
    for (size_t size = conductor->registering_resources.length, last_index = size - 1, i = size, index = i - 1; i > 0; i--, index--)
    {
        aeron_client_registering_resource_t *async_resource = conductor->registering_resources.array[index].resource;

        if (async_resource->registration_id == cmd->registration_id &&
            aeron_client_conductor_resource_type_match(async_resource->type, cmd->command_base.type))
        {
            if (NULL == aeron_int64_to_ptr_hash_map_get(&conductor->resource_by_id_map, cmd->registration_id))
            {
                if (AERON_CLIENT_MANAGED_RESOURCE_TYPE_SUBSCRIPTION == async_resource->type)
                {
                    result = aeron_client_conductor_offer_remove_subscription_command(
                        conductor, async_resource->registration_id);
                }
                else if (AERON_CLIENT_MANAGED_RESOURCE_TYPE_PUBLICATION == async_resource->type ||
                    AERON_CLIENT_MANAGED_RESOURCE_TYPE_EXCLUSIVE_PUBLICATION == async_resource->type)
                {
                    result = aeron_client_conductor_offer_remove_publication_command(
                        conductor, async_resource->registration_id, false);
                }
                else if (AERON_CLIENT_MANAGED_RESOURCE_TYPE_COUNTER == async_resource->type)
                {
                    result = aeron_client_conductor_offer_remove_counter_command(
                        conductor, async_resource->registration_id);
                }
            }
            ...
            aeron_client_conductor_async_resource_free(async_resource);
            break;
        }
    }
    ...
    if (NULL != resource)
    {
        aeron_client_managed_resource_type_t resource_type = resource->type;
        aeron_client_managed_resource_type_t command_type = cmd->command_base.type;

        if (aeron_client_conductor_resource_type_match(resource_type, command_type))
        {
            if (AERON_CLIENT_MANAGED_RESOURCE_TYPE_SUBSCRIPTION == resource_type)
            {
                result = aeron_client_conductor_on_cmd_close_subscription(conductor, resource);
            }
            else if (AERON_CLIENT_MANAGED_RESOURCE_TYPE_PUBLICATION == resource_type)
            {
                result = aeron_client_conductor_on_cmd_close_publication(conductor, resource);
            }
            else if (AERON_CLIENT_MANAGED_RESOURCE_TYPE_EXCLUSIVE_PUBLICATION == resource_type)
            {
                result = aeron_client_conductor_on_cmd_close_exclusive_publication(conductor, resource);
            }
            else if (AERON_CLIENT_MANAGED_RESOURCE_TYPE_COUNTER == resource_type ||
                    AERON_CLIENT_MANAGED_RESOURCE_TYPE_STATIC_COUNTER == resource_type)
            {
    ...
    void *on_complete_clientd)
{
    ...
    if (aeron_client_conductor_command_rb_offer_inline(conductor->command_rb, &cmd, sizeof(cmd)) < 0)
    {
        AERON_APPEND_ERR("%s", "");
        return -1;
    }
    ...
    void *on_close_complete_clientd)
{
    ...
    if (aeron_client_conductor_command_rb_offer(conductor->command_rb, counter) < 0)
    {
...
int aeron_client_conductor_async_handler(aeron_client_conductor_t *conductor, aeron_client_handler_cmd_t *cmd)
{
    ...
    while (aeron_client_conductor_command_rb_offer(conductor->command_rb, cmd) < 0)
    {
...
static const char* aeron_client_conductor_managed_resource_type_to_string(
    ...
    aeron_client_registering_resource_t *async)
{
    ...
    if (AERON_CLIENT_MANAGED_RESOURCE_TYPE_NEXT_AVAILABLE_SESSION_ID != resource_type)
    {
        *resource = NULL;
    }
            ...
            switch (resource_type)
            {
                ...
                case AERON_CLIENT_MANAGED_RESOURCE_TYPE_STATIC_COUNTER:
                    *resource = async->resource.counter;
                    break;
                case AERON_CLIENT_MANAGED_RESOURCE_TYPE_NEXT_AVAILABLE_SESSION_ID:
                    memcpy(resource, &async->resource.next_session_id, sizeof(int32_t));
                    break;
                default:
                    break;
            }
